...
public enum ErrorFunction: Codable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: ErrorFunction.CodingKeys.self)
        let type = try container.decode(Int.self, forKey: .type)
        let average = try container.decode(Bool.self, forKey: .average)
        
        switch type {
        case 1:     self = .default(average: average)
        case 2:     self = .crossEntropy(average: average)
            
        default:
            throw DecodingError.dataCorruptedError(forKey: ErrorFunction.CodingKeys.type, in: container, debugDescription: "invalid type value")
        }
    }
    
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: ErrorFunction.CodingKeys.self)
...
public struct FFNN: Codable {
    private enum CodingKeys: String, CodingKey {
        case inputs, hidden, outputs, momentum, learningRate, activationFunction, hiddenWeights, outputWeights, errorFunction
    }
    ...
    public init(inputs: Int, hidden: Int, outputs: Int, learningRate: Float = 0.7, momentum: Float = 0.4, weights: [Float]? = nil, activationFunction: ActivationFunction = .Default, errorFunction: ErrorFunction = .default(average: false)) {
        if inputs < 1 || hidden < 1 || outputs < 1 || learningRate <= 0 {
            fatalError("FFNN: invalid arguments passed to initializer inputs, hidden, outputs, and learningRate must be positive and nonzero")
        }
        ...
        if let weights = weights {
            guard weights.count == numHiddenWeights + numOutputWeights else {
                fatalError("FFNN: invalid number of weights provided")
            }
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: FFNN.CodingKeys.self)
        
        let inputs = try container.decode(Int.self, forKey: .inputs)
        let hidden = try container.decode(Int.self, forKey: .hidden)
        let outputs = try container.decode(Int.self, forKey: .outputs)
        ...
        let activationFunction = try container.decode(ActivationFunction.self, forKey: .activationFunction)
        let hiddenWeights = try container.decode([Float].self, forKey: .hiddenWeights)
        let outputWeights = try container.decode([Float].self, forKey: .outputWeights)
        let errorFunction = try container.decode(ErrorFunction.self, forKey: .errorFunction)
        let weights = hiddenWeights + outputWeights
        
        self.init(inputs: inputs, hidden: hidden, outputs: outputs, learningRate: learningRate, momentum: momentum, weights: weights, activationFunction: activationFunction, errorFunction: errorFunction)
    }
    
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: FFNN.CodingKeys.self)
        
        try container.encode(self.numInputs, forKey: .inputs)
        try container.encode(self.numHidden, forKey: .hidden)
        try container.encode(self.numOutputs, forKey: .outputs)
        try container.encode(self.learningRate, forKey: .learningRate)
        try container.encode(self.momentumFactor, forKey: .momentum)
        try container.encode(self.hiddenWeights, forKey: .hiddenWeights)
        try container.encode(self.outputWeights, forKey: .outputWeights)
        try container.encode(self.activationFunction, forKey: .activationFunction)
        try container.encode(self.errorFunction, forKey: .errorFunction)
    }
